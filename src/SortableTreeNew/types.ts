...
/**
 * Signature-compatible replacement for react-dnd's connect functions.
 * Takes a React element and returns a new element with the drag/drop behavior wired up.
 */
...
export type ReactSortableTreeProps = {
  ...
  /**
   * When true, or a callback returning true, dropping nodes to external
...
export type InjectedNodeRendererProps = {
  connectDragPreview: ConnectFunction;
  connectDragSource: ConnectFunction;
  didDrop: boolean;
  isDragging: boolean;
};
...
export type InjectedTreeProps = {
  ...
  canDrop?: boolean;
  /** Drop target */
  connectDropTarget: ConnectFunction;
  draggedNode?: TreeItem;
  isOver: boolean;
};
